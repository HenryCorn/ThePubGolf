...
export default async function TeamPage() {
  ...
  return (
    <div className="px-4 py-6 max-w-lg mx-auto">
      ...
      <div style={{ background: '#151a19', borderRadius: 14, padding: '1rem', border: '1px solid #2a3533', marginBottom: '1rem' }}>
        <p style={{ fontSize: '0.8rem', color: '#7a9390', marginBottom: 2 }}>You are</p>
        <p style={{ fontSize: '1.3rem', fontWeight: 700 }}>{me.emoji} {me.name}</p>
      </div>

      {!team ? (
        <div style={{ background: '#151a19', borderRadius: 14, padding: '1rem', border: '1px solid #2a3533' }}>
          <p style={{ color: '#7a9390' }}>You haven&apos;t been assigned to a team yet. Check back once the admin generates teams!</p>
        </div>
      ) : (
        <>
          <div style={{ background: '#151a19', borderRadius: 14, padding: '1rem', border: '1px solid #2a3533', marginBottom: '1rem' }}>
            <p style={{ fontSize: '0.8rem', color: '#7a9390', marginBottom: 4 }}>Team</p>
            <p style={{ fontSize: '1.2rem', fontWeight: 700, marginBottom: '0.75rem' }}>{team.name}</p>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {teammates.map((p) => (
                <span key={p.id} style={{
                  background: p.id === me.id ? '#00594F' : '#1e2523',
                  border: `1px solid ${p.id === me.id ? '#CEDC00' : '#2a3533'}`,
                  borderRadius: 99, padding: '4px 10px', fontSize: '0.85rem',
                }}>
                  {p.emoji} {p.name}
                  {team.captain_id === p.id && <span style={{ marginLeft: 4, color: '#CEDC00' }}>★</span>}
                </span>
              ))}
            </div>
          </div>
          ...
          <div style={{ background: '#151a19', borderRadius: 14, padding: '1rem', border: '1px solid #2a3533' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
              ...
              <span style={{ fontWeight: 800, fontSize: '1.3rem', color: '#CEDC00' }}>{total} pts</span>
            </div>
            {scores.length === 0 ? (
              <p style={{ color: '#7a9390', fontSize: '0.85rem' }}>No scores yet</p>
            ) : (
              scores.map((s: any) => (
                <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', padding: '4px 0', borderTop: '1px solid #2a3533' }}>
                  <span style={{ color: '#7a9390' }}>{s.stops?.position}. {s.stops?.pub_name}</span>
