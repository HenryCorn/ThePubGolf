  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '0.75rem', marginBottom: '1.5rem' }}>
        {stats.map((s) => (
          <div key={s.label} style={{
            ...
            border: '1px solid #2a3533', textAlign: 'center',
          }}>
            <div style={{ fontSize: '2rem', fontWeight: 800, color: '#CEDC00' }}>{s.value}</div>
            <div style={{ fontSize: '0.8rem', color: '#7a9390' }}>{s.label}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginBottom: '1.5rem' }}>
        ...
        <Link href="/leaderboard" style={{ ...linkStyle, borderColor: '#CEDC00', color: '#CEDC00' }}>
          🏆 View Live Leaderboard →
        </Link>
      </div>

      <div style={{ borderTop: '1px solid #2a3533', paddingTop: '1rem' }}>
        <p style={{ fontSize: '0.8rem', color: '#7a9390', marginBottom: '0.75rem' }}>
...
const linkStyle: React.CSSProperties = {
  ...
  background: '#151a19', border: '1px solid #2a3533',
  color: '#e8f0ee', textDecoration: 'none', fontWeight: 600,
}
